...
import { StarAlternateIcon, StarIcon } from '@bubbles-ui/icons/solid';
import React from 'react';
import { SubjectCardStyles } from './SubjectCard.style';

function SubjectImage({ subject }) {
  if (subject.image) {
    return <ImageLoader src={window.getUrl(subject.image)} radius={100} width={40} height={40} />;
  }

  if (subject.icon) {
    return <ImageLoader src={window.getUrl(subject.icon)} radius={100} width={40} height={40} />;
  }
